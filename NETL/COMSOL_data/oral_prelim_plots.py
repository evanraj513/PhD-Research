...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr 10 08:32:18 2021
...
def plot_segmented_data(plot_y, file, save_question = 0, plot_x = 0, file_name = 'Unnamed', plot_ideal=False, title_name = '',saveas = ''):
    '''
    ...
    if saving == 1:
        ...
        if saveas == '':
            ...
            pdf = matplotlib.backends.backend_pdf.PdfPages(saveas+'.pdf')
    ...
    power_ideal_hal = K*(1-K)*sig*u**2*B**2/(1+beta_i)
    
    try:
        x_plot = beta_i ## L_input
    except:
        # x_plot = df_hal[key_hal[0]]
        print('Error. Input not valid.')
        raise Exception
    
    x_plot_name = r'$\beta_i$ []'
    
    rs2 = int(mob_i.unique().size) ## Number of mob_i tested
    rs1 = int(beta_e.shape[0]/rs2)
    
    mob_e = mob_e.to_numpy().reshape(rs1,rs2)
    beta_e = beta_e.to_numpy().reshape(rs1,rs2)
    beta_i = beta_i.to_numpy().reshape(rs1,rs2)
    ...
    for a in np.arange(0,rs1):
        ## Ignoring the 0 for plotting purposes ##
        ax_hal[0].semilogx(x_plot[a][1:6], comsol_power[a][1:6],'-',
                       label = r'COMSOL: $\mu_e =$ '+ str(round(mob_e[a][0],2)),
                       color = color_list[a])

        ax_hal[0].semilogx(x_plot[a][1:6], power_ideal_hal[a][1:6], '-x',
                        label = r'Ideal Power $\mu_e =$ '+ str(round(mob_e[a][0],2)),
                        color = color_list[a])
        
        ax_hal[1].semilogx(x_plot[a][1:6], diff_hal[a][1:6],'-x', label=r'$\mu_e =$ '+str(round(mob_e[a][0],2)),
                       color = color_list[a])

    ax_hal[0].set_ylabel(y_plot_name)
    ax_hal[1].set_xlabel(x_plot_name)
    ax_hal[0].set_title(title_name)
    ax_hal[0].legend(fontsize = 'x-small', loc = 'upper center', bbox_to_anchor=(0.9, 1))
    ax_hal[1].set_ylabel('Relative Error')
    ax_hal[1].legend(fontsize='small')
...
file_name = 'seg_far_both_beta_sweep_2'
file='../comsol_data_v2/'+file_name

# file_name = 'evan_both_beta_seg_far_run2'
# file='../Imp_COMSOL_data/'+file_name

plot_segmented_data('ec.Ey*ec.Jy (W/m^3), Faraday Power', file, save_question=0, plot_x=0,
                    title_name='Segmented Faraday Power', plot_ideal=False)

def plot_continuous_data(plot_y, file, save_question = 0, plot_x = 0, file_name = 'Unnamed', plot_ideal=False, title_name = '',saveas = ''):
    '''
    Plots the data contained in the file provided. 
    '''
    ...
    for a in np.arange(0,rs1):
        ...
        ax_hal[1].semilogx(x_plot[a][1:6], diff_hal[a][1:6], label=r'$\mu_e =$ '+str(round(mob_e[a][0],2)),
                       color = color_list[a])
...
file_name = 'con_far_both_beta_sweep_2'
file='../comsol_data_v2/'+file_name

# file_name = 'evan_both_beta_seg_far_run2'
# file='../Imp_COMSOL_data/'+file_name

plot_continuous_data('ec.Ey*ec.Jy (W/m^3), Faraday Power', file, save_question=0, plot_x=0,
                    title_name='Continuous Faraday Power', plot_ideal=False)